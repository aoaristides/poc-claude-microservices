 ...
 *
 * <p>Publica de forma síncrona e só então marca como publicado: se o envio falhar,
 * a linha continua pendente e é retentada no próximo ciclo. Combinado com a dedup
 * do consumer (inbox no Orders), garante at-least-once seguro.
 *
 * <p>[inferência] Em volume maior, substituir o polling por CDC (Debezium) para
 * menor latência e menor carga no banco.
 */
@Component
class OutboxRelay {
    ...
    @Scheduled(fixedDelayString = "${shipping.outbox.relay-delay-ms:500}")
    @Transactional
